...
export interface CreateTokenParams {
  ...
  imageUrl: string;
  description: string;
  initialBuy?: bigint; // Optional initial buy amount in XLM
}
...
export class TokenFactoryService extends BaseContractService {
  constructor() {
    ...
    if (!contractId) {
      throw new Error(
        ...
        'Please set NEXT_PUBLIC_TOKEN_FACTORY_CONTRACT_ID in your .env.local file'
      );
    }
   ...
   * Returns the operation to be included in a transaction
   */
  buildCreateTokenOperation(params: CreateTokenParams): xdr.Operation {
    ...
    // Add initial buy amount if provided
